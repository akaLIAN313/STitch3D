...
class Model():
        ...
        for step in tqdm(range(self.training_steps)):
            ...
            if report_loss:
                if not step % step_interval:
                    print(
                        "Step: %s, Loss: %.4f, d_loss: %.4f, f_loss: %.4f" %
                        (step, loss.item(),
                         self.net.decon_loss.item(),
                         self.net.features_loss.item()))

    def eval(
            self, adata_st_list_raw: List[an.AnnData],
        ...
        if save == True:
            coor_3d = pd.DataFrame(
                data=self.adata_st.obsm['3D_coor'],
                index=self.adata_st.obs.index, columns=['x', 'y', 'z'])
    ...
    def cells_to_spatial(self, adata_ref_input,
        ...
        if celltype_ref is not None:
            if not isinstance(celltype_ref, list):
                ...
                adata_ref = adata_ref[[(t in celltype_ref)
                                       for t in adata_ref.obs
                                       [celltype_ref_col].values.astype(str)],
                                      :]
        else:
        ...
        for step in tqdm(range(training_steps_M)):
            ...
            generated_spots = torch.matmul(
                torch.transpose(M_hat, 0, 1),
                ref_counts)  # N_spots x G
            loss_sim_spots = - torch.mean(
                F.cosine_similarity(Y_adjusted, generated_spots, dim=1))
            loss_sim_genes = - torch.mean(
                F.cosine_similarity(Y_adjusted, generated_spots, dim=0))

            generated_spots_prop = torch.matmul(
                torch.transpose(M_hat, 0, 1),
                celltype_onehot)  # N_spots x C
            # Normalize generated proportions
            generated_spots_prop = generated_spots_prop / \
                torch.sum(M_hat, axis=0).view(-1, 1)
            ...
            loss_M = loss_prop + lam_sim * (
                loss_sim_spots + loss_sim_genes) + lam_num * reg_cell_num + lam_M * reg_M
